...
**Configuration.** `DATABASE_URL` (required). `WITNESS_STREAM` (default `purse`). `WITNESS_KEY_FILE` (required; PEM, PKCS8; generated on first run when absent, the public key logged and served). `REKOR_URL` (default `https://log2025-1.rekor.sigstore.dev`, documented as a rotating value). `REKOR_LOG_KEY` (required; `<origin>=<base64 DER>`, the pinned log key; the README shows where it comes from). `WITNESS_INTERVAL_MS` (default 300000). `WITNESS_MAX_LAG` (default 2, intervals). `WITNESS_PORT` (default 8082), `WITNESS_BIND` (default 0.0.0.0). `OTEL_EXPORTER_OTLP_*` as the broker. A misconfiguration is a fatal, specific error at boot.

**The tick.** Load the stream, `verifyChain`. Empty stream: nothing, readiness green. Chain broken: append a `witness_events` row `{ kind: "chain-broken", brokenAt, id, reason }`, anchor nothing, readiness red. Head equal to the last anchor's head: nothing. Head moved: `RekorV2.submit(stream, seq, head, signer)`, append the anchor, log one line with the log index. Rekor failure: append `{ kind: "anchor-failed", reason }`, retry next tick.

**Port.** Read-only, no token. `GET /` (service, version, stream, witness public key, log url and key id, interval). `GET /anchors?since=<seq>` (the anchors, newest last). `GET /verify` (runs `verifyAnchored` over the live chain and anchors with the pinned log key and its own witness key, returns the full result). `GET /events?since=<n>`. `GET /healthz` (200 when the process is up). `GET /readyz` (200 only when the last tick completed within `WITNESS_MAX_LAG` intervals, the chain verified, and either the head equals the last anchor's head or the last anchor is younger than `WITNESS_MAX_LAG` intervals; otherwise 503 with the reason).
...
**Telemetry.** Spans `deadlatch.witness.tick` and `deadlatch.witness.anchor`, counters `deadlatch.witness.anchors` and `deadlatch.witness.failures`, gauge `deadlatch.witness.lag` (receipts appended since the last anchor), through deadlatch-otel when the OTLP endpoint is set.
...
3. Fly reference deployment gains a witness machine with a persisted key file; its `GET /verify` is the public artefact. A log follow-up on deadlatch.dev shows a real anchor and the verifier call.
4. blackbox minor.
